...
def ladder(array):
    ...
    lad = [[array[0]]]  # Each ladder is a sublist
...
def merge_ladders(lists):
    merged = []
    heap = MinHeap()
    offsets = [0] * len(lists)  # simulate popleft with index offset

    for i, sublist in enumerate(lists):
        if sublist:
            heap.push((sublist[0], i))  # val, list_index
    ...
    while len(heap) > 0:
        ...
        if offsets[i] < len(lists[i]):
            next_val = lists[i][offsets[i]]
            heap.push((next_val, i))

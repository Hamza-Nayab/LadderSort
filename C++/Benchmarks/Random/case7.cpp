...
int main() {
    ...
    mt19937 rng(123);
    uniform_int_distribution<int> value_dist(1, 1'000'000);

    // Generate 10 unique random values
    unordered_set<int> unique_vals;
    while (unique_vals.size() < 10) {
        unique_vals.insert(value_dist(rng));
    }
    vector<int> random_values(unique_vals.begin(), unique_vals.end());
    ...
    for (int run = 1; run <= 10; ++run) {
        ...
        // Replace last 10 elements with unique random values
        for (int i = n - 10; i < n; ++i) {
            arr[i] = random_values[i - (n - 10)];
        }
    ...
    cout << "Average time (Sorted with last 10 unique random elements): "

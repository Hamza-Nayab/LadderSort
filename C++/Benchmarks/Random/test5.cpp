// g++ -O3 -std=c++17 -march=native bench_laddersort_ablation.cpp -o bench_laddersort_ablation
...
//=========================== Search ===========================================
// plain lower_bound on NON-INCREASING tops: first j with tops[j] <= x
...
// hinted lower_bound on NON-INCREASING tops: first j with tops[j] <= x
...
static void merge_k_heap(const std::vector<std::vector<ItemT<Stable>>>& runs,
    ...
    struct Cmp {
        bool operator()(const Node& a, const Node& b) const {
            // priority_queue is max-heap, so reverse comparison
...
//=========================== Dataset ==========================================
// Social-feed style fan-in: 24 nondecreasing sources, bursty, tie-heavy
...
int main() {
    ...
    for (const auto& C : cases) {
        ...
        auto r_full = bench_algo_postinsert("LadderFull", base, rounds, [&](std::vector<int>& v) {
            ladder_sort_into(v, g_ladder_out_ws, cfg_full);
            v.swap(g_ladder_out_ws);
        });

        auto r_no_hint = bench_algo_postinsert("NoHint", base, rounds, [&](std::vector<int>& v) {
            ladder_sort_into(v, g_ladder_out_ws, cfg_no_hint);
            v.swap(g_ladder_out_ws);
        });

        auto r_no_gallop = bench_algo_postinsert("NoGallop", base, rounds, [&](std::vector<int>& v) {
            ladder_sort_into(v, g_ladder_out_ws, cfg_no_gallop);
            v.swap(g_ladder_out_ws);
        });

        auto r_heap = bench_algo_postinsert("HeapMerge", base, rounds, [&](std::vector<int>& v) {
            ladder_sort_into(v, g_ladder_out_ws, cfg_heap);
            v.swap(g_ladder_out_ws);
        });

        auto r_stable = bench_algo_postinsert("StableMode", base, rounds, [&](std::vector<int>& v) {
        ...
        auto r_stdstable = bench_algo_postinsert("StableSort", base, rounds, [&](std::vector<int>& v) {

...
static void ladder_sort_into(const std::vector<int>& a, std::vector<int>& out) {
    ...
    // Optional fast-path for already sorted input.
    if (std::is_sorted(a.begin(), a.end())) {
        out = a;
        return;
    }

    const size_t K_THRESHOLD = (size_t)std::sqrt((double)a.size());
    ...
    for (int i = 1, n = (int)a.size(); i < n; ++i) {
        ...
        // Hybrid fallback: too many runs means adversarial pattern for LadderSort.
        if (lad.size() > K_THRESHOLD) {
            out = a;

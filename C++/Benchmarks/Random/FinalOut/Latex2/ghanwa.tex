...
\subsubsection {Prposition 1: Dilworth equality for sequences}
...
\subsubsection {Theorem 2: Optimality of the greedy ladder decomposition}
...
\subsection{Definition 1: Classical Presortedness Measures}
...
\subsection{Definition 4: Placement Index}
...
reducing comparison count on riffled interleavings.  When two ladders
...
\subsubsection{Lemma 1: Each ladder remains sorted}
...
\subsubsection{Lemma 2: Strict tail invariant}
...
\subsubsection{Lemma 3: Merge outputs a globally sorted list}
...
\subsubsection{Theorem 4: Correctness of LadderSort}
...
\subsubsection{Lemma 5: Time complexity}
...
\subsubsection{Preposition 7: Expected $K$ under uniform random permutation}
\label{prop:avgcase}
...
\subsubsection{Corollary 8: Average-case complexity}
\label{cor:avgcase}
...
\subsubsection{Preposition 9: Worst-case comparison lower bound under $K$-interleaving}
...
\subsubsection{Lemma 10: Space complexity}
\label{lem:space}
...
\begin{enumerate}
  \item {Refinements for duplicates and tie rules.}  Our correctness
  ...
  \item {Dynamic $K$-bounding.}  Can the fallback threshold of Hybrid
  ...
  \item {In-place variants.}  LadderSort's $O(N+K)$ space requirement
  ...
  \item {Lower bounds for the $K$-adaptive model.}  The comparison
  ...
  \item {Parallel and external-memory LadderSort.}  Phase~1 is

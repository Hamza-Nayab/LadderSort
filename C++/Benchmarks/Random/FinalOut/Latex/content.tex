...
\centering
\small
\caption{Measured ladder count \(K\) for the main benchmark workloads. Values are means over 10 runs. The table confirms that the benchmark suite spans the intended structural range, from \(K=1\) to \(K\approx N\).}
...
\caption{Controlled \(K\)-sweep at \(N=10^7\) using randomized exact-\(K\) interleavings. Median runtime generally increases with the measured ladder count \(K\), consistent with LadderSort's \(O(N\log K)\) parameterization. TimSort remains strongest at \(K=1\), where the input is already one sorted run.}
...
\caption{Speedup relative to TimSort on the controlled randomized exact-\(K\) sweep. LadderSort and Hybrid LadderSort are strongest in the small-\(K\) interleaved regime, while their advantage narrows as \(K\) grows.}
...
\centering
\small
\caption{Phase 4 ablation results. Entries are median relative time normalized to LadderFull \(=1.00\) for the same workload and input size. Values above 1.00 are slower than LadderFull, while values below 1.00 are faster.}
...
\centering
\scriptsize
\caption{Hybrid LadderSort evaluation. Speedups are based on median runtime. The fallback rate is the fraction of runs in which Hybrid LadderSort switched to its fallback path.}
...
\centering
\small
\caption{Peak resident set size at \(N=10^8\), reported in GiB. The \(10^8\) memory experiment uses one seed as a large stress case; \(N=10^7\) measurements use three seeds and are retained in the result pack.}
...
\centering
\small
\caption{Median key comparisons per element at \(N=10^6\). Lower values indicate fewer key comparisons. Small-\(K\) interleaved workloads require far fewer comparisons for LadderSort than general-purpose baselines.}
...
\centering
\small
\caption{Real-world GH Archive event-trace benchmark. The trace contains 55,364 records from 12,783 repository sources. Times are medians over 10 rounds. Speedup is relative to TimSort.}

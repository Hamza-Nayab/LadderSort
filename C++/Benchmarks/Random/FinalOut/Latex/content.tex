...
Raw LadderSort stores ladders and merge state, which is costly when \(K=N\): descending input reaches 3.73~GiB. Hybrid falls back and reduces that case to 1.12~GiB. On social-feed input, \(K=24\), so Hybrid keeps the raw path and has the same footprint.

Table~\ref{tab:phase5-comparisons} reports median key comparisons per element at \(N=10^6\) over five seeds.
...
The table separates structure from raw speed. Raw LadderSort measures \(K=368\), so the trace is far from random-like under the ladder decomposition. Raw LadderSort is faster than TimSort, and Hybrid is faster still because it takes the conservative fallback path. At this small input size, however, \texttt{std::sort} and \texttt{std::stable\_sort} remain faster. The real-trace result is therefore evidence of low-\(K\) structure and TimSort-level competitiveness, not a universal speed claim.
...
LadderSort depends on the measured ladder count \(K\). When \(K\approx N\), as in descending input, raw LadderSort reaches its \(O(N\log N)\) worst case with larger constants than tuned in-place baselines. It is also not in-place: storing ladders and merge state costs \(O(N+K)\) auxiliary space, and high-\(K\) construction can scatter writes across many ladders.

The synthetic workloads are designed to model common patterns, not to cover all data. Different fan-in, burst, duplicate, or key-size choices could shift constants. The real-trace experiment adds one public GH Archive trace, but it should not be read as evidence that all real event streams have low measured \(K\).

Implementation choices also matter. The TimSort baseline is one header-only implementation, while \texttt{std::sort} and \texttt{std::stable\_sort} depend on the standard library, allocator, and platform. Our experiments use one ARM-based laptop, fixed seeds, and a documented compiler command; cross-platform results may differ.

Hybrid LadderSort reduces high-\(K\) exposure but introduces a threshold that may need tuning. Future work should evaluate more traces and key types, add a stable variant, reduce memory traffic, and study threshold policies across platforms.

\section{\texorpdfstring{\textbf{Conclusions and Future Work}}{Conclusions and Future Work}}\label{conclusions-and-future-work}

We introduced LadderSort, a deterministic adaptive sorting algorithm for inputs that are interleavings of nondecreasing subsequences. LadderSort differs from natural run-adaptive methods such as TimSort by adapting to the number of interleaved nondecreasing subsequences \(K\), rather than the number of contiguous runs. The raw algorithm is the main contribution: it builds ladders online and merges them in \(O(N\log K)\) time using \(O(N+K)\) auxiliary space.

The experiments support the intended use case. LadderSort is strongest on small-\(K\) interleavings, remains competitive but not dominant on random input, and exposes its raw worst case on descending input. The ablation, memory, comparison-count, and GH Archive experiments clarify the engineering costs and the structural regime where the method is useful. Hybrid LadderSort is best viewed as a practical guardrail for high-\(K\) inputs, not a replacement for the raw \(K\)-adaptive algorithm.

Future work should broaden the real-trace evaluation, develop stable and lower-memory variants, and tune fallback thresholds. Parallel and external-memory variants are also natural directions because the ladder representation already separates the input into sorted streams.
